...
Page({
    ...
    toUploadVideo(videoFile) {
      ...
      cancelUpload() {
        this.data.v_uploader.cancel();
    },
    resumeUpload() {
        this.data.v_uploader.start();
    },
  ...
  */
    ...
    deleteVideo() {
        wx.showModal({
            ...
            success: (res) => {
                if (res.confirm) {
                    if (this.data.video.id || this.data.video.fileId) {
                      this.deleteOnlineVideo(this.data.video.id||this.data.video.fileId);
                      // 飞流方式
    ...
    deleteOnlineVideo(id) {
      ...
      let params = {
          url: app.API_HOST + "ShortVideo/delTenYunVideoOne",
          data: {
              id: id,
...
*/
    uploadCardVideo(data) {
        let url = this.data.id ? "ShortVideo/updateShortVideo" : "ShortVideo/addShortVideo";
        let params = {
            url: app.API_HOST + url,
            data: data
        }
        this.data.id && ( params.data.id = this.data.id );
        fetchApi(this, params).then(res => {
    ...
    submit() {
        let data = {
            name: this.data.title.replace(/(^\s*)|(\s*$)/g, ""),
            share: this.data.content,
        }
        if (data.name.length < 1) {
            showTips("请填写视频标题", this);
            return "";
        }
        if (data.share.replace(/(^\s*)|(\s*$)/g, "") < 1) {
            showTips("请填写分享语", this);
            return "";
        }
        if (this.data.video.fileId||this.data.video.id) {
          data.address = this.data.video.videoUrl||this.data.video.url;
        // if (this.data.video.status === 2) { 飞流方式
            // data.address = this.data.video.url;
            data.duration = this.data.video.duration;
            data.videoId = this.data.video.id;
            data.fileId = this.data.video.fileId||this.data.video.id;
            data.width = this.data.video.width;
            data.height = this.data.video.height;
           } else if (!this.data.video.fileId && !this.data.video.id) {
        // } else if (this.data.video.status === 0) { 飞流方式
            showTips("请上传视频", this);
            return "";
        } else {
        ...
        if (this.data.videoCover.status === 2) {
            data.img_id = this.data.videoCover.id;
        ...
        if (this.data.havaGoods) {
            if (this.data.goods.link_goods_id != 0) {
                data.link_goods_id = this.data.goods.link_goods_id;
            } else {
                showTips("请选择商品", this);
                return "";
            }
    ...
    getCardVideoInfo() {
        ...
        fetchApi(this, params).then(res => {
            ...
            this.setData({
                title: data.name,
                content: data.share,
                textLength: data.share.length,
                video: {
                    // id: data.video_id, 飞流方式
                    id: data.id,
                    url: data.address,
    ...
    choiceRecommendGoods() {
        const params = {
            url: `/shortVideo/recommendGoodsList/recommendGoodsList`,
            data: {
