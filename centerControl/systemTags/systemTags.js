...
Page({
    ...
    tagActive(e) {
        ...
        for (let i in this.data.tagList) {
            for (let j in this.data.tagList[i]) {
                if (item.name == this.data.tagList[i][j].name) {
                    this.data.tagList[i][j].is_select = this.data.tagList[i][j].is_select ? 0 : 1
                } else if (item.type == this.data.tagList[i][j].type) {
                    this.data.tagList[i][j].is_select = 0
                }
                this.dataStore.messageTag.push(this.data.tagList[i][j])
            }
        }
        this.setData({
    ...
    pcTagActive(e) {
        ...
        this.data.pcData[index].tagItem.forEach((item, cIndex) => {
            if (cIndex == itemindex) {
                this.data.pcData[index].tagItem[cIndex].is_select = item.is_select == 1 ? 0 : 1
            }else {
                this.data.pcData[index].tagItem[cIndex].is_select = 0
            }
        })
        this.setData({
            pcData: this.data.pcData
        })
    },
    saveTag() {
        this.setData({
        ...
        let pcTags = []
        //console.log("xq6666"+this.data.pcData)
        if (this.data.pcData != undefined && this.data.pcData.length != 0) {
            //console.log("xq5555"+this.data.pcData)
            this.data.pcData.forEach(i => {
                i.tagItem.forEach(j => {
                    j.is_select == 1 && pcTags.push(j.id)
                })
            })
        }
        ...
        let param = {
            url: `crmUser/saveCrmUserTag`,
            data: {
                tags: this.dataStore.tags,
                crmUid: this.dataStore.crmUid,
                tagItemList: JSON.stringify(pcTags)
            }
        }
        fetchApi(this, param).then(res => {
    ...
    getTagData() {
        let param = { 
            url: `crmUser/getCrmUserTagWithMapNew`,
            data: {
                crmUid: this.dataStore.crmUid
            }
        }
        fetchApi(this, param).then(res => {
            let commData = res.data.data.user
            let pcData = res.data.data.corp
            let _myTag = commData[0] instanceof Array ? {} : commData[0]   //我的标签无数据时后台返回[]，将其转化为{}
            this.data.myTag = _myTag
            this.dataStore.myTag = deepClone(_myTag)
            delete commData[0] 
            this.data.tagList = commData
            this.dataStore.tagList = deepClone(commData)
            this.setData({
                myTag: this.data.myTag,
                tagList: this.data.tagList,
                pcData: pcData
